  ...
  return (
    ...
    >
      ...
      {connected ? <>
        ...
        <div className='text-5xl mb-4' onClick={getDeposit}>{deposit || deposit.toFixed(2)} BNB
        {/* <span className='text-lg font-bold'>BSC</span> */}
        </div>

        <p className='mb-4 font-light text-sm'>{formatAddress(signerAddress!)}</p>

        <div>
            <button onClick={handleOpenDeposit} className='p-4 px-8 rounded-full bg-black text-white'> Deposit</button>
            {!isContractCreator && <button onClick={handleOpenAddAsset} className='ml-4 p-4 px-8 rounded-full bg-black text-white '> New Asset</button>}
        </div>
      </div>
